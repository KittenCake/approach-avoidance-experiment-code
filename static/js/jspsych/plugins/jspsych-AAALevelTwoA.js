...
jsPsych.plugins["AAALevelTwoA"] = (function() {
...
plugin.trial = function(display_element, trial){        // for every trial should a object be made
  ...
  trial.players = ['/static/images/imageTokken.jpeg','/static/images/imageSleepingRobber.jpeg','/static/images/imagePlayer.jpeg', '/static/images/imageCatchRobberRed.jpeg','/static/images/imageTokkenLevelTwo.jpeg'];
  ...
  if (trial.finish == false && trial.playerGotCaught == true){
    ...
    trial.finishNormal = false;
     ...
     var trial_data = {
      ...
      var interceptX = jStat.gamma.sample(2,1);
      drawnInterceptInterval = Math.min(Math.max(parseInt(interceptX), interceptX), 4)*1000;
      var remainingTokenTime = (trial.dissapearTokkenTime[5]*1000) - ((new Date()).getTime() - trial.startTime)
...
function checkFullscreen(trialData){  // checks after every trial if fullscreen is on or not -> trial only ends if fullscreen is on again
  ...
  display_element.append($('<img>', {           // level 2 normal level indicator everything the same as in level 1
  src: trial.players[4],
  id: 'jspych-AAALevel2IndicateDiamond'}));
